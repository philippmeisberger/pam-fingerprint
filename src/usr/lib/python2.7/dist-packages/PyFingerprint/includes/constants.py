"""
"" PyFingerprint
""
"" Copyright 2014 Bastian Raschke.
"" All rights reserved.
"""

## Baotou start byte
FINGERPRINT_STARTCODE = 0xEF01

## Packet identification
##

FINGERPRINT_COMMANDPACKET = 0x01

FINGERPRINT_ACKPACKET = 0x07
FINGERPRINT_DATAPACKET = 0x02
FINGERPRINT_ENDDATAPACKET = 0x08

## Instruction codes
##

FINGERPRINT_VERIFYPASSWORD = 0x13
FINGERPRINT_SETPASSWORD = 0x12
FINGERPRINT_SETADDRESS = 0x15
FINGERPRINT_SETSYSTEMPARAMETER = 0x0E
FINGERPRINT_GETSYSTEMPARAMETERS = 0x0F
FINGERPRINT_TEMPLATECOUNT = 0x1D

FINGERPRINT_READIMAGE = 0x01
FINGERPRINT_CONVERTIMAGE = 0x02

FINGERPRINT_COMPARECHARACTERISTICS = 0x03
FINGERPRINT_DOWNLOADCHARACTERISTICS = 0x08 ## Note: The documention says 0x09 but it's not correct

FINGERPRINT_CREATETEMPLATE = 0x05
FINGERPRINT_STORETEMPLATE = 0x06
FINGERPRINT_LOADTEMPLATE = 0x07
FINGERPRINT_DELETETEMPLATE = 0x0C

FINGERPRINT_CLEARDATABASE = 0x0D
FINGERPRINT_SEARCHTEMPLATE = 0x04

## Packet reply confirmations
##

FINGERPRINT_OK = 0x00
FINGERPRINT_COMMUNICATIONERROR = 0x01

FINGERPRINT_WRONGPASSWORD = 0x13

FINGERPRINT_INVALIDREGISTER = 0x1A

FINGERPRINT_NOFINGER = 0x02
FINGERPRINT_IMAGEERROR = 0x03

FINGERPRINT_MESSYIMAGE = 0x06
FINGERPRINT_FEATUREFAIL = 0x07
FINGERPRINT_INVALIDIMAGE = 0x15

FINGERPRINT_ENROLLMISMATCH = 0x0A

FINGERPRINT_BADLOCATION = 0x0B
FINGERPRINT_FLASHERROR = 0x18

FINGERPRINT_READERROR = 0x0C
FINGERPRINT_OUTOFRANGE = 0x0B

FINGERPRINT_DELETEERROR = 0x10

FINGERPRINT_CLEARERROR = 0x11

FINGERPRINT_NOMATCHFOUND = 0x09






FINGERPRINT_ADDRCODE = 0x20
FINGERPRINT_PASSVERIFY = 0x21

FINGERPRINT_UPLOADFAIL = 0x0F

FINGERPRINT_PACKETRESPONSEFAIL = 0x0E

FINGERPRINT_UPLOADFEATUREFAIL = 0x0D
FINGERPRINT_DBRANGEFAIL = 0x0C

FINGERPRINT_NOMATCH = 0x08
FINGERPRINT_NOTFOUND = 0x09

## General error codes
##



FINGERPRINT_TIMEOUT = 0xFF
FINGERPRINT_BADPACKET = 0xFE
